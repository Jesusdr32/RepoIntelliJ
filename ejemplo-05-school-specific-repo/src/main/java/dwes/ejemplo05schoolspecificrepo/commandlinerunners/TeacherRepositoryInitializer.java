...
import dwes.ejemplo05schoolspecificrepo.services.TeacherService;
...
public class TeacherRepositoryInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        for (int i = 0; i < TEACHER_COUNT; i++) {
            Teacher teacher = Teacher.builder().firstName("Nombre" + i).lastName("Apellido" + i).email(String.format("emailProfesor%s@iesclaradelrey.es", i)).build();
